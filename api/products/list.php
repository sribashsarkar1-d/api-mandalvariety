...
try {
    ...
    foreach ($products as &$product) {
        ...
        $images = json_decode($imgStr, true);
        
        $full_images = [];
        if (is_array($images)) {
            $full_images = array_map(function($img) use ($baseUrl) {
                $img = (string)$img;
                if (filter_var($img, FILTER_VALIDATE_URL) || strpos($img, 'http') === 0) return $img;
                return $baseUrl . ltrim($img, '/');
            }, $images);
        } else {
            // Fallback if not json
            if (!empty($imgStr) && strpos((string)$imgStr, 'http') !== 0) {
                $full_images = [$baseUrl . ltrim((string)$imgStr, '/')];
            }
        }
...
try {
    ...
    echo json_encode(['success' => true, 'data' => $products], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
} catch (Exception $e) {
    http_response_code(200); // Return 200 so Flutter prints the message
